...
fn main() {
    ...
    println!("Virtual Screen: {}x{} px (from {}, {} to {}, {})", 
        width, height, left, top, left + width, top + height);
    
    println!("Primary Monitor: {}x{} px", 
    ...
    for (hwnd, window_info) in &tracker.windows {
        ...
        println!("  HWND: {:?}", hwnd);
        println!("  Position: ({}, {}) to ({}, {})", rect.left, rect.top, rect.right, rect.bottom);
        println!("  Size: {}x{}", width, height);
        println!("  Monitor: {}", monitor_area);
        println!("  Grid cells: {} cells -> {:?}", window_info.grid_cells.len(), 
            if window_info.grid_cells.len() <= 6 { &window_info.grid_cells } else { &window_info.grid_cells[..6] });
        println!();
    }
    ...
    for (hwnd, window_info) in &tracker.windows {
        let rect = &window_info.rect;
        if rect.left >= 5120 {
            println!("SECOND MONITOR WINDOW: {} at ({}, {})", 
