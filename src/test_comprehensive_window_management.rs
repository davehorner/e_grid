...
impl WindowManager {
    fn new(monitor_rect: RECT) -> Self {        Self {
            windows: HashMap::new(),
            monitor_rect,
    ...
    fn scan_and_update_windows(&mut self) {
        ...
        let stale_windows: Vec<HWND> = self.windows
            .iter()
        ...
        let in_grid: Vec<HWND> = self.windows
            .iter()
            .filter(|(_, w)| w.in_grid)
            .map(|(&hwnd, _)| hwnd)
            .collect();
            
        let mut not_in_grid: Vec<HWND> = self.windows
            .iter()
            .filter(|(_, w)| !w.in_grid)
            .map(|(&hwnd, _)| hwnd)
            .collect();
    ...
    fn animate_to_grid_layout(
        ...
        println!("   🎯 Windows selected for grid: {}", selected_windows.len());
        ...
        let windows_to_remove: Vec<HWND> = self.windows
            .iter()
        ...
        for &hwnd in &windows_to_remove {
            let (original_rect, title) = {
              ...
              println!("  📤 Sending '{}' back to original position", 
                &truncate_string_safe_bytes(&title, 25));
                
            self.animate_window_to_position(hwnd, original_rect, animation_duration_ms, easing.clone());
        ...
        for (i, &hwnd) in selected_windows.iter().enumerate() {
            if let Some(window) = self.windows.get(&hwnd) {
                println!("      {}. '{}'", i + 1, &truncate_string_safe_bytes(&window.title, 30));
            }
        }
        ...
        for (i, &hwnd) in selected_windows.iter().enumerate() {
            ...
            let target_rect = self.calculate_grid_position(target_row, target_col, grid_rows, grid_cols);
            ...
            self.animate_window_to_position(hwnd, target_rect, animation_duration_ms, easing.clone());
        }
        
        // Update rotation index for next time (based on non-grid windows)
        let available_for_rotation = self.windows
            .iter()
            .filter(|(_, w)| !w.in_grid)
            .count();
        ...
        if available_for_rotation > 0 {
            self.next_rotation_index = (self.next_rotation_index + selected_windows.len()) % available_for_rotation;
        }
        
        println!("   🔄 Rotation index: {} → {} (available: {})", 
    ...
    fn calculate_grid_position(&self, row: usize, col: usize, grid_rows: usize, grid_cols: usize) -> RECT {
    ...
    fn animate_window_to_position(&mut self, hwnd: HWND, target_rect: RECT, duration_ms: u64, easing: EasingType) {
        if let Some(window) = self.windows.get_mut(&hwnd) {
            println!("   🎯 Animating window '{}' to position [{},{} {}x{}]", 
    ...
    fn wait_for_animations(&mut self, duration_ms: u64) {
      ...
      fn rotate_grid_windows(&mut self, grid_rows: usize, grid_cols: usize, rotation_steps: usize, step_duration_ms: u64) {
        let grid_size = grid_rows * grid_cols;
        
        for step in 0..rotation_steps {
            println!("\n🔄 ROTATION STEP {} of {} (scanning for new windows)", step + 1, rotation_steps);
            ...
            if all_windows.len() < grid_size {
                println!("   ⚠️  Only {} windows available for {}-cell grid", all_windows.len(), grid_size);
            }
            ...
            let current_in_grid: Vec<HWND> = self.windows
                .iter()
                .filter(|(_, w)| w.is_in_grid())
                .map(|(&hwnd, _)| hwnd)
                .collect();
            
            println!("   📊 Available windows: {}, Grid size: {}, Will rotate: {}", 
                all_windows.len(), grid_size, num_to_rotate);
            
            // Step 1: Remove some current grid windows
            let windows_to_remove = current_in_grid.iter().take(num_to_rotate).cloned().collect::<Vec<_>>();
            ...
            for &hwnd in &windows_to_remove {
                ...
                println!("  📤 Rotating out: '{}'", 
                    &truncate_string_safe_bytes(&title, 20));
                    
                self.animate_window_to_position(hwnd, original_rect, step_duration_ms / 2, EasingType::EaseOut);
            ...
            for i in 0..grid_size {
                ...
                self.animate_window_to_position(hwnd, target_rect, step_duration_ms, EasingType::EaseIn);
            }
            
            // Update rotation index for next step
            self.next_rotation_index = (self.next_rotation_index + num_to_rotate) % all_windows.len();
            
            // Wait for enter animations
            self.wait_for_animations(step_duration_ms);
            
            println!("   ✅ Rotation step {} complete (next index: {})", step + 1, self.next_rotation_index);
    ...
    fn find_available_grid_slot(&self, grid_rows: usize, grid_cols: usize) -> Option<(usize, usize)> {
        for row in 0..grid_rows {
            for col in 0..grid_cols {
                let slot_occupied = self.windows
                    .values()
    ...
    fn restore_all_windows(&mut self, animation_duration_ms: u64) {
        ...
        for (i, (_hwnd, window)) in self.windows.iter().enumerate() {            println!("  📦 Restoring window {}: '{}'", 
                i + 1,
        ...
        for (hwnd, original_rect) in windows_to_restore {
            self.animate_window_to_position(hwnd, original_rect, animation_duration_ms, EasingType::EaseInOut);
        }
      ...
      fn print_status(&self, phase: &str) {
        ...
        println!("   Windows available for rotation: {}", available_for_rotation);
        println!("   Windows animating: {}", animating_count);
        println!("   Next rotation index: {}", self.next_rotation_index);
    }
    
    fn progressive_grid_fill(&mut self, grid_rows: usize, grid_cols: usize, phase_name: &str, step_duration_ms: u64) {
        println!("\n🎬 {}: Progressive fill of {}x{} grid", phase_name, grid_rows, grid_cols);
        ...
        for step in 0..grid_size.min(all_windows.len()) {
            println!("\n   🔄 FILL STEP {} of {}", step + 1, grid_size.min(all_windows.len()));
            ...
            self.shift_grid_windows_and_add_new(grid_rows, grid_cols, next_window, step_duration_ms);
        ...
        self.next_rotation_index = (self.next_rotation_index + grid_size) % all_windows.len().max(1);
        
        println!("   ✅ Progressive grid fill complete!");
    }
    
    fn shift_grid_windows_and_add_new(&mut self, grid_rows: usize, grid_cols: usize, new_window: HWND, duration_ms: u64) {
        let grid_size = grid_rows * grid_cols;
        
        // Get current windows in grid (in order of their positions)
        let mut current_grid_windows: Vec<(HWND, usize, usize)> = self.windows
            .iter()
        ...
        if current_grid_windows.len() >= grid_size {
            if let Some((last_window, _, _)) = current_grid_windows.pop() {
                if let Some(window) = self.windows.get(&last_window) {
                    ...
                    self.animate_window_to_position(last_window, original_rect, duration_ms, EasingType::EaseOut);
                }
      ...
      fn wait_for_user_input_with_scanning(&mut self, prompt: &str) {
        ...
        for i in 0..10 {  // Check 10 times over 5 seconds
            let initial_count = self.windows.len();
            self.scan_and_update_windows();
            let new_count = self.windows.len();
            
            if new_count > initial_count {
                println!("   🆕 Detected {} new window(s)! Total: {}", 
                    new_count - initial_count, new_count);
...
fn main() {
    ...
    window_manager.wait_for_user_input_with_scanning("    Press Enter to start the comprehensive window management demo...");// Phase 1: Start with 2x2 Grid with immediate layout (for testing)
    ...
    window_manager.print_status("PHASE 1 - AFTER SCAN");
      window_manager.wait_for_user_input_with_scanning("⏸️  Press Enter to see window rotation in 2x2 grid...");
