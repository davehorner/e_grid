...
impl MonitorGrid {
    pub fn new(monitor_id: usize, config: GridConfig) -> Self {
        ...
        Self {
            ...
            windows: HashMap::new(),
            cells,
            monitor_bounds: (0, 0, 0, 0), // Default bounds, should be updated with real values
        }
    }
    ...
    pub fn print_grid(&self) {
        ...
        for row in &self.cells {
            ...
            for cell in row {
                match cell {
                    Some(hwnd) => line.push_str(&format!("{:04X} ", (*hwnd as u64) & 0xFFFF)),
                    None => line.push_str(" --- "),
                }
            }
            println!("{}", line);
    ...
    pub fn window_to_grid_cells(&self, rect: &RECT) -> Vec<(usize, usize)> {
        let (left, top, right, bottom) = self.monitor_bounds;
