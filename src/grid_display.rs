...
pub fn display_grid(
    ...
    if let Some(focused) = focused_hwnd {
        println!(
            "DEBUG: Focused window HWND: 0x{:X} (last 2 digits: {:02X})",
            focused,
            focused & 0xFF
        );
            ...
            for col in 0..config.cols {
                if let CellState::Occupied(hwnd) = grid[row][col] {
                    if hwnd == focused {
                        found_in_grid = true;
                        println!(
                            "DEBUG: Focused window FOUND in grid at row {} col {}",
                            row, col
                        );
                        break;
                    }
    ...
    if topmost_hwnd.is_some() || focused_hwnd.is_some() {
        ...
        if let Some(topmost) = topmost_hwnd {
            println!(
                "  \x1b[31mRed\x1b[0m = Topmost window (HWND: 0x{:X})",
                topmost
            );
        }
        if let Some(focused) = focused_hwnd {
            println!(
                "  \x1b[34mBlue\x1b[0m = Input focus window (HWND: 0x{:X})",
                focused
            );
        }
    }
