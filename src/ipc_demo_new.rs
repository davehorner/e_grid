...
fn run_client() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let event_service = match node
        ...
        Err(e) => {
            println!("❌ [CLIENT {}] Failed to connect to event service: {}", client_id, e);
            return Err(e.into());
        }
    };
    ...
    let command_service = node
      ...
      let command_publisher = match command_service.publisher_builder().create() {
        Ok(publisher) => {
            println!("✅ [CLIENT {}] Connected to command service as publisher", client_id);
            Some(publisher)
        },
        Err(e) => {
            println!("⚠️ [CLIENT {}] Could not create command publisher (limit reached): {}", client_id, e);
            println!("   This client will be read-only (events/responses only)");
            None
        }
    };
      println!("✅ [CLIENT {}] Connected to all IPC services", client_id);
    println!("📡 [CLIENT {}] Listening for events and responses...", client_id);
    if command_publisher.is_some() {
        println!("💬 [CLIENT {}] Commands: 'g' = get grid, 'w' = get windows, 'q' = quit", client_id);
    } else {
        println!("📖 [CLIENT {}] Read-only mode - no commands available", client_id);
    }
    ...
    loop {
        ...
        while let Some(response_sample) = response_subscriber.receive()? {
            ...
            println!("📤 [CLIENT {}] Received Response: {:?}", client_id, response);
            ...
            match response.response_type {
                ...
                2 => {
                    ...
                    println!("   🔍 Grid display would show {} windows in grid layout", response.window_count);
                }
                3 => {
                    // Grid State response
                    println!("   📊 GRID STATE:");
                    ...
                    println!("   ├─────────────────────────────────────┤");
                    println!("   │ Windows: {} | Cells: {}             │", response.window_count, response.data[0]);
            ...
            if std::io::stdin().read_line(&mut input).is_ok() {
                ...
                if !input.is_empty() {
                    match input.as_str() {
                        ...
                        _ => {
                            println!("❓ [CLIENT-{}] Unknown command '{}'. Type 'h' for help.", client_id, input);
                        }
                    }
                }
            }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ Client terminal spawned with PID: {}", client_process.id());
    ...
    println!("   • Client process spawned with PID: {}", client_process.id());
    ...
    loop {
        ...
        match client_process.try_wait() {
            Ok(Some(status)) => {
                ...
                println!("   Try running the client manually: cargo run --bin ipc_demo_new -- --client");
            },
