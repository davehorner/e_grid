...
use e_grid::{WindowTracker, GridConfig, EasingType};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut server = GridIpcServer::new(tracker.clone())?;
    ...
    // Let's manually scan for existing windows to see if any are detected
    {
        ...
        println!("🔍 Found {} total windows after scan", tracker_guard.windows.len());
...
fn demonstrate_grid_animation(client: &mut GridClient, tracker: &Arc<Mutex<WindowTracker>>, server: &mut GridIpcServer) -> Result<(), Box<dyn std::error::Error>> {
    println!("   📐 Using IPC commands to arrange windows in 2x2 grid...");
      // Get current windows from tracker
    let window_list = {
        let tracker_guard = tracker.lock().unwrap();
        tracker_guard.windows.iter().map(|entry| *entry.key()).collect::<Vec<_>>()
    };
    ...
    for (i, &hwnd) in window_list.iter().take(4).enumerate() {
        ...
        println!("   📦 Moving window {:?} (as u64: {}) to grid position [{},{}]", hwnd, hwnd as u64, row, col);
...
fn demonstrate_window_rotation(client: &mut GridClient, tracker: &Arc<Mutex<WindowTracker>>, server: &mut GridIpcServer) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for rotation in 1..=3 {
        ...
        for (i, &hwnd) in window_list.iter().take(4).enumerate() {
            ...
            let row = base_pos / 2;
            let col = base_pos % 2;            println!("     📦 Moving window {:?} (as u64: {}) to [{},{}]", hwnd, hwnd as u64, row, col);
            client.move_window_to_cell(hwnd as u64, row as u32, col as u32)?;
...
fn demonstrate_realtime_events(tracker: &Arc<Mutex<WindowTracker>>, server: &mut GridIpcServer) -> Result<(), Box<dyn std::error::Error>> {
