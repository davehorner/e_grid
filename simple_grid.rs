        ...
        if let Ok(mut tracker) = tracker_arc.try_lock() {
            match msg {                WM_CREATE => {
                    ...
                    if let Some(register_fn) = register_shell_hook {
                        ...
                        println!("Real-time updates may not work, but periodic updates will continue");
                    }
                    
                    tracker.shell_hook_id = RegisterWindowMessageW(
                        b"SHELLHOOK\0".as_ptr() as *const u16
                    );
                        ...
                        1 => { // HSHELL_WINDOWCREATED
                            if WindowTracker::is_manageable_window(window_hwnd) {
                                if tracker.add_window(window_hwnd) {
                                    println!("Window created: {}", WindowTracker::get_window_title(window_hwnd));
                                    tracker.print_grid();
                                }
                            }
                        }
                        ...
                        4 => { // HSHELL_WINDOWACTIVATED
                            if WindowTracker::is_manageable_window(window_hwnd) {
                                if tracker.update_window(window_hwnd) {
                                    println!("Window activated: {}", WindowTracker::get_window_title(window_hwnd));
                                    tracker.print_grid();
                                }
                            }
                        }
