...
fn animate_window_to_position(
    ...
    println!(
        "    🎬 Animating window '{}' to [{},{} {}x{}] over {}ms with {:?} easing",
        if window.title.len() > 20 {
            &window.title[..20]
        } else {
            &window.title
        },
...
fn animate_windows_to_grid(
    ...
    println!(
        "   ✅ Animation complete! All windows positioned in {}x{} grid",
        grid_rows, grid_cols
    );
}
...
fn rotate_windows_in_grid(
    ...
    for step in 0..rotation_steps {
        ...
        for (i, window) in windows.iter_mut().enumerate().take(max_windows) {
            ...
            let target_rect =
                calculate_grid_position(monitor_rect, new_row, new_col, grid_rows, grid_cols);
