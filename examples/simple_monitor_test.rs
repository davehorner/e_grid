...
use winapi::um::winuser::{
    EnumDisplayMonitors, GetSystemMetrics, SM_CXVIRTUALSCREEN, SM_CYVIRTUALSCREEN,
...
fn main() {
    ...
    println!(
        "🖥️ Virtual Screen: {}x{} at ({},{})",
        virtual_rect.2 - virtual_rect.0,
        virtual_rect.3 - virtual_rect.1,
        virtual_rect.0,
        virtual_rect.1
    );
            ...
            for row in [0, 2, 4] {
                for col in [0, 3, 6, 9] {
                    ...
                    for (i, monitor_rect) in monitors.iter().enumerate() {
                        if cell_left < monitor_rect.2
                            && cell_right > monitor_rect.0
                            && cell_top < monitor_rect.3
                            && cell_bottom > monitor_rect.1
                        {
                            monitor_id = Some(i);
                            break;
                        }
                    ...
                    match monitor_id {
                        Some(id) => println!(
                            "   Cell ({},{}) -> Monitor {} at ({},{})",
                            row, col, id, cell_left, cell_top
                        ),
                        None => println!(
                            "   Cell ({},{}) -> OFF-SCREEN at ({},{})",
                            row, col, cell_left, cell_top
                        ),
                    }
                }
            }
