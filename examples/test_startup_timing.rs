use e_grid::{ipc_client::ClientCellState, GridClient};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if startup_time.as_secs_f64() <= 3.0 {
        println!(
            "\n✅ STARTUP TIMING TEST PASSED - Completed in {:.2}s (≤ 3s)",
            startup_time.as_secs_f64()
        );
    } else {
        println!(
            "\n❌ STARTUP TIMING TEST FAILED - Took {:.2}s (> 3s)",
            startup_time.as_secs_f64()
        );
    }
    ...
    if monitor_data.len() > 0 && window_data.len() > 0 {
        ...
        println!(
            "❌ DATA AVAILABILITY TEST FAILED - Missing data (monitors: {}, windows: {})",
            monitor_data.len(),
            window_data.len()
        );
    }
    ...
    let total_monitor_windows: usize = monitor_data
        .iter()
        .map(|m| {
            m.grid
                .iter()
    ...
    if total_monitor_windows > 0 {
        println!(
            "✅ MONITOR GRID TEST PASSED - {} windows found in monitor grids",
            total_monitor_windows
        );
    } else {
