use e_grid::{ipc, EasingType, WindowTracker};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let layout_service = node
        ...
        .open()?;
    let mut layout_publisher = layout_service.publisher_builder().create()?;
    ...
    let cell_service = node
        ...
        .open()?;
    let mut cell_publisher = cell_service.publisher_builder().create()?;
...
fn get_window_list(publisher: &mut Publisher<iceoryx2::service::ipc::Service, ipc::WindowCommand, ()>) -> Result<(), Box<dyn std::error::Error>> {
...
fn save_current_layout(publisher: &mut Publisher<iceoryx2::service::ipc::Service, ipc::WindowCommand, ()>) -> Result<(), Box<dyn std::error::Error>> {
...
fn apply_layout_animated(publisher: &mut Publisher<iceoryx2::service::ipc::Service, ipc::WindowCommand, ()>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    publisher.send_copy(command)?;
    println!("✅ Sent ApplyGridLayout command for '{}' with {}ms duration and easing {}", 
             layout_name, duration_ms, easing_type);
    Ok(())
}

fn animate_specific_window(publisher: &mut Publisher<iceoryx2::service::ipc::Service, ipc::AnimationCommand, ()>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    publisher.send_copy(command)?;
    println!("✅ Sent animation command for window {} to ({}, {}) size {}x{} over {}ms", 
             hwnd, target_x, target_y, target_width, target_height, duration_ms);
    Ok(())
}

fn get_animation_status(publisher: &mut Publisher<iceoryx2::service::ipc::Service, ipc::AnimationCommand, ()>) -> Result<(), Box<dyn std::error::Error>> {
...
fn stop_all_animations(publisher: &mut Publisher<iceoryx2::service::ipc::Service, ipc::AnimationCommand, ()>) -> Result<(), Box<dyn std::error::Error>> {
...
fn check_responses(subscriber: &mut Subscriber<iceoryx2::service::ipc::Service, ipc::WindowResponse, ()>) -> Result<(), Box<dyn std::error::Error>> {
    while let Some(sample) = subscriber.receive()? {
        let response = *sample;
        match response.response_type {
            0 => println!("✅ Server: Success"),
            1 => println!("❌ Server: Error (code {})", response.error_code),
            2 => println!("📋 Server: Window list contains {} windows", response.window_count),
            3 => println!("📊 Server: Grid state - {} windows, {} occupied cells", 
                        response.window_count, response.data[0]),
            4 => println!("🗂️ Server: {} saved layouts available", response.window_count),
            5 => println!("🎬 Server: Animation status - {} animations ({} active)", 
                        response.data[0], response.data[1]),
            _ => println!("❓ Server: Unknown response type {}", response.response_type),
        }
    }
    Ok(())
}
