...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = client.start_background_monitoring();
    let elapsed = start_time.elapsed();

    match result {
        Ok(_) => {
            println!(
                "✅ start_services() completed successfully in {:.2}s",
                elapsed.as_secs_f64()
            );
        }
        Err(e) => {
    ...
    if elapsed.as_secs_f64() <= 3.5 {
        // Allow some margin
        println!(
            "✅ TIMEOUT TEST PASSED - Completed in {:.2}s (≤ 3.5s)",
            elapsed.as_secs_f64()
        );
    } else {
        println!(
            "❌ TIMEOUT TEST FAILED - Took {:.2}s (> 3.5s)",
            elapsed.as_secs_f64()
        );
    }
    ...
    println!(
        "  ✅ Timeout behavior: REASONABLE ({:.2}s)",
        elapsed.as_secs_f64()
    );
