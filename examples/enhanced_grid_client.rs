use e_grid::{
    EventType, GridClient, GridClientConfig, GridClientError, OperationTimer, PerformanceMonitor,
};
use std::sync::Arc;
use std::thread;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if config.focus_events.enabled {
        ...
        grid_client.set_focus_callback(move |focus_event| {
            ...
            println!("🎯 {} window {}", event_type, focus_event.hwnd);
        })?;
...
fn demonstrate_configuration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Grid size: {}x{}",
        loaded_config.grid.rows, loaded_config.grid.cols
    );
