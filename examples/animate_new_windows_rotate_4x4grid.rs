...
use e_grid::window_events::{WindowEventConfig, run_message_loop};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (tracker_req_tx, tracker_req_rx) = mpsc::channel::<Box<dyn FnOnce(&mut WindowTracker) + Send>>();
    ...
    let event_callback = move |event: e_grid::ipc_protocol::GridEvent| {
        ...
        if let e_grid::ipc_protocol::GridEvent::WindowCreated { hwnd, .. } = event {
            ...
            tracker_req_tx_cb.send(Box::new(move |mut tracker: &mut WindowTracker| {
                // Scan windows
