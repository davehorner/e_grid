...
fn test_different_grid_sizes() {
    ...
    for (rows, cols, name) in test_configs {
        ...
        println!("✅ Created WindowTracker with {}x{} grid", config.rows, config.cols);
...
fn test_grid_bounds(config: &GridConfig) {
    ...
    for (row, col) in valid_positions {
        if row < config.rows && col < config.cols {
            ...
            println!("     ❌ Position ({}, {}) should be valid but isn't", row, col);
        }
    }
    ...
    for (row, col) in invalid_positions {
        if row >= config.rows || col >= config.cols {
            ...
            println!("     ❌ Position ({}, {}) should be invalid but isn't", row, col);
        }
    }
}
...
fn test_ipc_server_client() {
    ...
    for (i, config) in server_configs.iter().enumerate() {
        println!("\n🖥️  Test {}: Server with {}x{} grid", i + 1, config.rows, config.cols);
        ...
        println!("   📋 Server config: rows={}, cols={}", config.rows, config.cols);
...
fn test_server_with_config(config: GridConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let server = GridIpcServer::new(tracker)?;
    println!("   🔧 Server initialized with {}x{} grid", server.get_config().rows, server.get_config().cols);
    Ok(())
}
...
fn test_client_initialization() {
    ...
    match GridClient::new() {
        Ok(client) => {
            ...
            println!("   📐 Client grid: {}x{}", client.get_config().rows, client.get_config().cols);
                ...
                match serde_json::from_str::<GridConfig>(&json) {
                    Ok(deserialized) => {
                        if deserialized.rows == config.rows && deserialized.cols == config.cols {
                            ...
                            println!("   ❌ Deserialization mismatch: {}x{} vs {}x{}", 
