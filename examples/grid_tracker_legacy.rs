...
impl WindowTracker {
    ...
    fn window_to_grid_cells(&self, rect: &RECT) -> Vec<(usize, usize)> {
        ...
        let end_col =
            ((rect.right - monitor.rect.left) / cell_width).min(GRID_COLS as i32 - 1) as usize;
        let start_row = ((rect.top - monitor.rect.top) / cell_height).max(0) as usize;
        let end_row =
            ((rect.bottom - monitor.rect.top) / cell_height).min(GRID_ROWS as i32 - 1) as usize;
