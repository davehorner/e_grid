...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if monitor_data.is_empty() {
        ...
        //     println!(
        //         "ℹ️  Manual get_monitor_list() returns {} monitors",
...
fn display_individual_monitor_grids(
    ...
    let monitor_list = client.get_monitor_list();
    if !monitor_list.is_empty() {
        println!(
            "✅ Using monitor list from get_monitor_list(): {} monitors",
            monitor_list.len()
        );
        ...
        for monitor in &monitor_list {
            ...
            println!(
                "=== MONITOR {} GRID ({}) ===",
                monitor.monitor_id, monitor_type
            );
            println!(
                "Monitor bounds: ({}, {}) to ({}, {})",
                monitor.x,
            ...
            for row in 0..config.rows.min(32) {
                ...
                for col in 0..config.cols.min(32) {
                    ...
                    match server_value {
                        Some(0) => print!(".. "),                        // Empty
                        Some(hwnd) if hwnd == u64::MAX => print!("XX "), // Offscreen
                        Some(hwnd) => {
                            let display = format_hwnd_with_color(hwnd, top_3);
                            print!("{} ", display);
                        }
                        None => print!(".. "), // Treat None as empty
                    }
                }
                println!();
            }
            ...
            let window_count: usize = (0..config.rows.min(32))
                ...
                .filter(|&hwnd_opt| match hwnd_opt {
                    Some(hwnd) if hwnd != 0 && hwnd != u64::MAX => true,
                    _ => false,
                })
                .count();
    ...
    let monitor_data = client.get_monitor_data();
    println!(
        "📊 Fallback: Using monitor data from get_monitor_data(): {} monitors",
        monitor_data.len()
    );
    ...
    for monitor in &monitor_data {
        ...
        let window_data = client.get_window_data();
        let windows_on_monitor: usize = window_data
            .iter()
            .filter(|window_info| window_info.monitor_id == monitor.monitor_id)
            .count();
